...
const getSignleTask = async(req,res) => {
    ...
    noTaskError(singleTask);
    res.status(200).json({singleTask});
}

const updateTask = async(req,res) => {
    const { id } = req.params;
    const updateTask = await Tasks.findOneAndUpdate({_id: id}, req.body, {new:true, runValidators: true});
    noTaskError(updateTask);
...
const deleteTask = async(req,res) => {
    ...
    noTaskError(deleteTask);
    res.status(200).json(await Tasks.find({}))
}

const noTaskError = (task) => {
    if (!task) {
